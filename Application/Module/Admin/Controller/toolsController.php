<?php
  ...
  namespace Application\Module\Admin\Controller;
	...
	class toolsController extends \Vendor\Controller\Controller {
      ...
      // Presmerovanie po neuspesnom prihlaseni
		 ...
		 * @return Void
		 */			
		public function renderHysteresis()
		{
			if ($this->registry->user->getLoggedIn() === FALSE)	{
				// Presmerovanie po neuspesnom prihlaseni
				$this->route->redirect();
        ...
        $select = array('*');
        // z tabulky Articles
        $from = array(\Application\Config\Settings::$Detail->Mysql->Table_Tools);
        // podla zhody id s parametrom v url
        $where = array(
                  array('=', \Application\Config\Settings::$Detail->Mysql->Table_Tools.'.Id'=>1)
                 );
        // zorad podla Title
        $order = array(\Application\Config\Settings::$Detail->Mysql->Table_Tools.'.Title');
        // vytvor poziadavku
